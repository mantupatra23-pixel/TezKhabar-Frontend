    ...
    <div className="bg-white min-h-screen text-neutral-900 antialiased font-sans flex flex-col justify-between">
      <div>
        ...
        <header className="bg-white border-b border-neutral-200 sticky top-0 z-50">
          <div className="max-w-3xl mx-auto px-4 pt-5 pb-3 flex flex-row justify-between items-end">
            <div>
          ...
          {/* DYNAMIC NAVIGATION HORIZONTAL BAR */}
          <div className="bg-white border-t border-neutral-100">
            <div className="max-w-3xl mx-auto px-2 flex gap-1 overflow-x-auto scrollbar-none">
              {categories.map((cat) => (
                <button
                  key={cat}
        ...
        <main className="max-w-2xl mx-auto px-4 py-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-36 gap-2">
              ...
              <p className="text-neutral-400 font-mono text-[9px] tracking-widest uppercase">Syncing Dashboard Stream...</p>
            </div>
          ) : newsList.length === 0 ? (
            <div className="text-center py-20 text-neutral-400 font-mono text-xs bg-white border border-neutral-200 rounded-xl">
              📭 Active pool stack empty. Scraper pipeline idle.
            </div>
          ) : (
            <div className="space-y-8">
              
              {/* BRIEF HOOK RE-LINKS (NDTV TRENDING MATRIX ACCENT) */}
                ...
                {newsList.map((news, index) => {
                  ...
                  return (
                    <article key={index} className="py-8 first:pt-0 last:pb-0 flex flex-col">
                      ...
                      {/* Premium Media Layout Frame */}
                      {verifiedImage && (
                        <div className="w-full aspect-video bg-neutral-100 rounded-xl overflow-hidden mb-5">
                          <img
                            src={verifiedImage}
                            alt="TezKhabar Broadcast Asset Feed"
                      ...
                      <h2 className="text-2xl sm:text-3xl font-black tracking-tight text-neutral-950 leading-tight font-serif hover:text-[#cc0000] transition-colors">
                        {cleanText(news.title)}
                      </h2>
                      
                      <p className="mt-4 text-neutral-700 text-sm sm:text-base font-normal leading-relaxed text-justify tracking-wide whitespace-pre-line">
                        {cleanText(news.content)}
                      </p>

                      {/* Action Triggers Footer */}
                      <div className="mt-5 flex justify-between items-center text-[10px] font-mono text-neutral-400 select-none">
